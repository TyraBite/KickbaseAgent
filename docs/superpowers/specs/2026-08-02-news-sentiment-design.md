...
**Doc-Id-Schema (korrigiert nach Live-Smoke-Test-Fund im finalen Whole-Branch-Review, 2026-08-02):** `{player_id}_{article_hash}` (`article_hash` = kurzer Hash über den Artikel-Link, per `hashlib.sha1(link.encode()).hexdigest()[:12]`) — **`date` (Lauf-Tag) ist bewusst NICHT Teil des Keys**, entgegen der ursprünglichen Fassung dieses Dokuments. Der erste Entwurf setzte `{date}_{player_id}_{article_hash}` und behauptete "idempotent bei täglichem Rerun" — das stimmt nur für Reruns am SELBEN Tag. Da derselbe Artikel bis zu `NEWS_LOOKBACK_DAYS`-1 weitere Tage erneut in der RSS-Antwort auftaucht, hätte die alte Formel ihn als bis zu 7 verschiedene Dokumente gespeichert (im Live-Test gemessen: ~640 Schreibungen/Tag, nur ~91 davon echte neue Artikel) — genau die Verzerrung, die dieses Feld eigentlich verhindern sollte, plus eine ~7-fache Übergewichtung in Phase C's späterer Aggregation. Mit `{player_id}_{article_hash}` als Key bekommt derselbe Artikel über alle Tage hinweg dasselbe Dokument (echtes Cross-Day-Dedup), `date` wird bei jedem erneuten Sehen als reines Feld überschrieben (harmlos, sogar nützlich als "zuletzt gesehen am"). Firestore-Dokument-Felder unverändert: `player_id`, `date` (Tag DIESES Laufs, nicht `pub_date` des Artikels — Konsistenz mit dem Diff-Log-Muster aus Phase A, wo `date` ebenfalls der Lauf-Tag ist), `pub_date` (Original-Veröffentlichungsdatum des Artikels, separat gespeichert), `headline`, `snippet`, `link`, `sentiment_label`, `sentiment_score`.
